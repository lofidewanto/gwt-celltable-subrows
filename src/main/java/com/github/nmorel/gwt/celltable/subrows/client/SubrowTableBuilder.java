...
public class SubrowTableBuilder extends MyDefaultCellTableBuilder<Parent> {
	...
	private void buildChildrenRow(Parent parent, int absRowIndex) {
		if (parent.isDeployed()) {
			...
			SelectionModel<? super Parent> selectionModel = cellTable
					.getSelectionModel();
			boolean isSelected = (selectionModel == null || parent == null) ?
					false :
					selectionModel.isSelected(parent);
			...
			for (Child child : parent.getChildren()) {
				buildChildRow(parent, i++, child, isEven, isSelected,
						trClasses);
			}
		}
	}

	protected void buildChildRow(Parent parent, int subrowIndex, Child child,
			boolean isEven, boolean isSelected, StringBuilder trClasses) {
         ...
         * number column
         */
		Context context = new Context(baseContext.getIndex(), 1, child,
				baseContext.getSubIndex());
         ...
         * date column
         */
		context = new Context(baseContext.getIndex(), 2, child,
				baseContext.getSubIndex());
		td = initChildTd(tr, evenOrOddStyle, 2, isSelected);
		div = td.startDiv();
		div.style().outlineStyle(OutlineStyle.NONE).endStyle();
		ChildCellColumn<Parent, Child, Date> dateColumn = new ChildCellColumn<Parent, Child, Date>(
				subrowIndex, childGetter, createDateCell(), dateValueGetter,
         ...
         * text column
         */
		context = new Context(baseContext.getIndex(), 3, child,
				baseContext.getSubIndex());
		td = initChildTd(tr, evenOrOddStyle, 3, isSelected);
		div = td.startDiv();
		div.style().outlineStyle(OutlineStyle.NONE).endStyle();
		ChildCellColumn<Parent, Child, String> textColumn = new ChildCellColumn<Parent, Child, String>(
				subrowIndex, childGetter, createTextCell(), textValueGetter,
		...
		div = td.startDiv();
		div.style().outlineStyle(OutlineStyle.NONE).endStyle();
		ChildCellColumn<Parent, Child, String> actionColumn = new ChildCellColumn<Parent, Child, String>(
				subrowIndex, childGetter, createActionCell(), textValueGetter,
				new FieldUpdater<Child, String>() {
					@Override
					public void update(int index, Child object, String value) {
						object.setText(value);
		...
		renderCell(div, context, actionColumn, parent);
		div.endDiv();
		td.endTD();
	...
	private TableCellBuilder initChildTd(TableRowBuilder tr,
			String evenOrOddStyle, int columnIndex, boolean isSelected) {
	...
	public ActionCell createActionCell() {
		ActionCell.Delegate delegate = new ActionCell.Delegate() {
			@Override
			public void execute(Object o) {
				logger.info("Action called...");
			}
		};
		return new ActionCell("Action", delegate);
	}
